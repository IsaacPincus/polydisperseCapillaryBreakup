function [a,Az,Ar,f,stress,ep,Wi,t,te,ye,ie] = ...
    filamentThinningFENEPM_inertia_initialEXP(chi, X, mu, rho, ...
        Nm, Ns, Lm, lambda_ms, gm, cutoff, tmax, texp, Dexp)
...
%     indices = indices(:);
    ...
    [y0_exp, a0] = getStretchExperiments(texp, Dexp, ...
        Nm, Ns, Lm, lambda_ms);

    y0(1) = a0;
    y0(2:2*Ntot+1) = y0_exp;
    ...
    tic
    [t,y,te,ye,ie] = ode15s(@(t,y) odefun(t,y,constants, Lm, lambda_ms, gm), ...
        [t0, tmax], y0, options);
    toc
...
function dydt = odefun(t, y, constants, L, lambda, g)
    Nm = constants(1);
    Ns = constants(2);
    chi = constants(3);
    mu = constants(4);
