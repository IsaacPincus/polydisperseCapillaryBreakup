function [a,Az,Ar,f,stress,ep,Wi,t,te,ye,ie] = ...
    filamentThinningFENEPM_inertia_CDD(chi, X, mu, rho, ...
        Nm, Ns, Lm, Nk, hstar, lambda_ms, gm, a0, cutoff, tmax, phieq)
...
%     indices = indices(:);
    ...
    [t,y,te,ye,ie] = ode15s(@(t,y) odefun(t,y,constants,...
        Lm, lambda_ms, Nk, hstar, gm, phieq), ...
        [t0, tmax], y0, options);
    toc
...
%     Ar = reshape(y(:,2*indices+1),[lent,Nm,Ns]);
    ...
    for mm=1:Nm
        for ss = 1:Ns
            ...
            count = count + 1;
            Ar(:,mm,ss) = y(:,count);
            % only apply to longest relaxation time
            if ss == 1
                for tt = 1:length(Az(:,mm,ss))
                    Q = sqrt((Az(tt,mm,ss) + 2*Ar(tt,mm,ss))/3);
                    zet(tt,mm) = zetacal(Q,d,phieq,Nk(mm),hstar,himodel);
                end
            end
        end
    end
...
%     f = sum(L2'./(L2'+3-Az-2*Ar),3)/Ns;
    ...
    Wi = lambda_ms(:,1).*ep'.*zet';

end

function dydt = odefun(t, y, constants, L, lambda, Nk, hstar, g, phieq)
    Nm = constants(1);
    Ns = constants(2);
    chi = constants(3);
    mu = constants(4);
...
%     Art = reshape(y(2*indices+1),[Ns,Nm])';
    ...
    for mm=1:Nm
        for ss = 1:Ns
            ...
            % only apply to longest relaxation time
            if ss == 1
                zet(mm,ss) = zetacal(Q,d,phieq,Nk(mm),hstar,himodel);
            else
                zet(mm,ss) = 1;
            end
        end
